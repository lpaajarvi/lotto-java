...
public class LottoUI {
    ...
    public static void main(String [] args) {
        ...
        timeline=askTimeline();
         This code is modified from the original Lotto.java. The first 3 lines here are rmemoved
        * we are going to add a new method to help determining if the user given parameters are ok
        */
        
        if (SomethingWrongWithTheUserGivenParameters(args)) {
            System.out.println("Please give "+NUMBERS+" unique lotto numbers as parameters, seperating them with space, which are between "+MIN+" and "+MAX);
        } else {
    ...
    private static boolean SomethingWrongWithTheUserGivenParameters(String[] args) {
        
        if (args.length != NUMBERS) {
            System.out.println("You did not provide "+NUMBERS+" different paramters.");
            return true;
        }
        for (int i=0; i<NUMBERS; i++) {
            ...
            if (currentNumber > MAX || currentNumber < MIN) {
                System.out.println(currentNumber+ " is not a number between "+MIN+" and "+MAX+".");
                return true;
            }
            if (!Arrays.contains(currentNumber, lottoSheet)) {
                lottoSheet[i]=currentNumber;
            } else {
                System.out.println("You tried to include "+currentNumber+" more than once in your lotto sheet.");
                return true;
            }
        }
        return false;
